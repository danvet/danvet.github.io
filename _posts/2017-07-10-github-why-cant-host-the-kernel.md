...
- Maintainer-Stuff
---
A while back at the awesome [maintainerati](https://maintainerati.org/) a
...
boils down to "... why don't these dinosaurs use modern dev tooling?". Despite
that a few top kernel maintainers vigorously defend mailing lists and patch
submissions over something like github pull requests at least a few folks from
the graphics subsystem would love more modern tooling. The problem is that
...
Git is awesome, because everyone can fork and create branches and hack around
...
awesome, because it figured out a UI that makes this complex stuff all nice&easy
...
all the pull requests and issues on a repository, and it's time to split things
...
support, but you can't yet delete them. In short you need to split up your
humongous project into sub-projects, each with their own flavour of process and
criteria and their own repo with their own pull request and issue tracking.
Generally it takes a few tens to few hundreds of full time contributors until
the pain is big enough that such a huge reorg is necessary.
...
dwelve on the benefits. But I'd like to highlight some of the issues this is
causing:
...
- Even once you've noticed the need for it, refactoring and code sharing have
  ...
  this dead is. But it is more effort than a simple pull request to the one
  ...
  repos for single functiosn, but then you essentially replace git with npm as
...
- The combinatorial explosion of theoretically supported version mixes becomes
  ...
  "please upgrade everything first". That again just that you still have your
  monorepo, except once more it's not managed in git anymore. Well, except if
  you use submodules, and I'm not sure that's considered git ...
...
- Reorganizing how you split the overall projects into sub-projects is a pain,
  ...
  initial step from a monorepo to a group of split up repositories.
 
## Interlude: Why Pull Requests Exist

The linux kernel is one of the few projects I'm aware of which isn't split up
like this. Before we look at how thats - the kernel is a huge project and
...
Still, the very first version of git supported pull requests, and the audience
of these first, rather rough, releases was kernel maintainers, because git was
written to solve kernel maintainer problems, so clearly it was needed and
...
subsystem of the kernel, or synchronize different sub-projects with a code
refactoring or similar cross-cutting change. As an example, the [4.12 network
pull request from Dave S. Miller](https://lkml.org/lkml/2017/5/2/508),
[committed by
...
don't commit in shared repositories is also why git tracks the committer and
author separately.
...
- All this madness is managed through the
  ...
  and the get_maintainers.pl script, which for any given snippet of code can
...
At first this just looks like an complicated way to fill everyone's disk space
...
- It's really really easy to reassign discussions on pull requests and issues
  ...
  threading are the same for everyone), despite that the mails are archive in a
  pile of different mailing list archive, go through different mailing lists and
...
- Cross-subsystem work doesn't need any kind of release dance. You simply change
  ...
  strictly more powerful than what a split repo setup allows you, because for
  ...
  once without causing too bug coordination pains.
...
- It doesn't prevent you from creating your own experimental additions, which is
  one of the key benefits of the multi-repo setup. You just for the kernel, add
  your code and leave it at that - no one ever forces you to push the code back,
...
because part a certain point they just stop scaling.
...
any git repo that's just for private use of one single contributor.
...
multiple repositories simultanously, with the one single discussion stream
shared among them all. Since this is the kernel, this step is done through patch
submission with a pile of different mailing lists and maintainers on the
recipient list.
...
of changes, and which would both need to resolve some non-trivial merge
...
patches, based on a commit common to both subsystems, is merged into *both*
...
spanned both the graphics and sound driver subsystems. The same commits where
...
In my opinion that's the single one reason why the kernel commit cannot even
consider moving to github - there's also the minor issue of a few top
maintainers being extremely outspoken against github in general, but that's a
not really a technical issue. But it's not just the Linux Kernel, it's all huge
...
Branches wouldn't do, since the point of splitting repos is also that pull
requests and issues are separated, too.
...
subsystem will pull it in. Only that subsystem will then merge and close the
pull. And again, a pull request might get merged multiple times, in each
subsystem with a different merge commit.
